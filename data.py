...
class TrashDataset(Dataset):
    def __init__(self, root_dir, transform=None, mode='train'):
        ...
        self.train_img_file = os.path.join(root_dir, 'data\\one-indexed-files-notrash_train.txt')
        self.val_img_file = os.path.join(root_dir, 'data\\one-indexed-files-notrash_val.txt')
        self.test_img_file = os.path.join(root_dir, 'data\\one-indexed-files-notrash_test.txt')

        img_dirs = [x[0] for x in os.walk(os.path.join(root_dir, 'data\\dataset-resized'))][1:]
        img_dirs_dict = {}
        
        for img_dir in img_dirs:
            trash_name = os.path.basename(img_dir)
